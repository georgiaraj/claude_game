...
class ClaudePlayer(Player):
    ...
    def make_move(self, game: CompetitiveGame, player_name: str) -> GameMove:
        ...
        prompt = self._create_strategy_prompt(
            game_description, game_state, valid_moves, player_name
        )
    ...
    def _create_strategy_prompt(self, game_description: str, game_state: Dict, valid_moves: List[Dict], player_name: str) -> str:
...
class GameSession:
    ...
    def play_turn(self, human_move: GameMove = None) -> Dict[str, Any]:
        ...
        # Get the move
        if player.type == "claude":
            move = self.claude_players[current_player].make_move(self.game, current_player)
        elif player.type == "human" and human_move:
            move = human_move
        else:
            return {"status": "waiting_for_human_move", "current_player": current_player}

        # Apply the move
        move_result = self.game.apply_move(move)
        ...
        if win_check:
            ...
            result["winner"] = win_check

        return result

    def play_full_game(self, max_turns: int = 100) -> Dict[str, Any]:
        """Play a full game automatically (for AI vs AI)"""
        self.start_game()
        results = []

        for turn in range(max_turns):
            if self.game.state.phase == GamePhase.FINISHED:
                break
        ...
        return {
            "final_state": self.game.state.to_dict(),
...
class TicTacToe(CompetitiveGame):
    ...
    def apply_move(self, move: GameMove) -> Dict[str, Any]:
        ...
        return {"success": True, "placed": symbol, "position": position}
...
def demo_claude_vs_claude():
    """Demo of Claude playing against itself"""

    # Create game
    config = {"name": "tic_tac_toe", "rules": {}}
    game = TicTacToe(config)

    # Create session
    session = GameSession(game)

    # Add two Claude players with different strategies
    player1 = Player("Claude-Aggressive", "claude", strategy="aggressive")
    player2 = Player("Claude-Defensive", "claude", strategy="defensive")

    session.add_player(player1)
    session.add_player(player2)

    # Play full game
    print("=== Claude vs Claude Tic-Tac-Toe ===")
    result = session.play_full_game()

    print(f"Game completed in {result['total_turns']} turns")
    final_state = result['final_state']

    if final_state['phase'] == 'finished':
        winner_info = None
        for move_result in reversed(result['move_history']):
            if move_result.get('winner'):
                winner_info = move_result['winner']
                break

        if winner_info and winner_info.get('winner'):
...
def demo_human_vs_claude():
    """Demo setup for human vs Claude"""
    config = {"name": "tic_tac_toe", "rules": {}}
    game = TicTacToe(config)
    session = GameSession(game)
    ...
    claude = Player("Claude", "claude", strategy=strategy, stats={"skill_level": skill_level})

    session.add_player(human)
    session.add_player(claude)

    session.start_game()

    print("=== Human vs Claude Tic-Tac-Toe ===")
    print("Enter moves as 'row,col' (0-2), e.g., '1,1' for center")
...
if __name__ == "__main__":
    ...
    # demo_claude_vs_claude()
    demo_human_vs_claude()
